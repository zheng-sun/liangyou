	...
	<div class="xxl_top_5">
        ...
        <?php $n=1;if(is_array($data)) foreach($data AS $r) { ?>
			<li>
				...
				<div class="zx_message top5_message"><a href="#" ><?php echo str_cut($r[description],156,'...');?></a></div>
			</li>
        <?php $n++;}unset($n); ?>
		</ul>
...
<div class="content">
    ...
    <div class="xxl_clear"></div>
	<div class="zx-con">
    <?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"get\" data=\"op=get&tag_md5=c662a4cb265dff3d2c94abdb85dd3e10&sql=select+%2A+from+v9_news+where+catid+%3D+%24catid+order+by+id+desc&start=5&num=5&cache=3600&page=%24page&return=data\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">编辑</a>";}pc_base::load_sys_class("get_model", "model", 0);$get_db = new get_model();$pagesize = 5;$page = intval($page) ? intval($page) : 1;if($page<=0){$page=1;}$offset = ($page - 1 ) * $pagesize + 5;$r = $get_db->sql_query("SELECT COUNT(*) - 5 as count FROM (select * from v9_news where catid = $catid order by id desc ) T");$s = $get_db->fetch_next();$pages=pages($s['count'], $page, $pagesize, $urlrule);$r = $get_db->sql_query("select * from v9_news where catid = $catid order by id desc LIMIT $offset,$pagesize");while(($s = $get_db->fetch_next()) != false) {$a[] = $s;}$data = $a;unset($a);?>
		<ul>
        <?php $n=1;if(is_array($data)) foreach($data AS $r) { ?>
			<li>
				<div class="zx-con-1">
				...
				<div class="zx-con-2">
					<div class="con_title"><a href="<?php echo $r['url'];?>" ><?php echo $r['title'];?></a></div>
					<div class="xxl_line"></div>
					<div class="con_time"><a href="<?php echo $r['url'];?>" ><?php echo date('Y年m月d日',$r[inputtime]);?></a></div>
					<div class="con_message"><?php echo str_cut($r[description],356,'...');?></div>
				</div>
				<div class="clear"> </div>
			</li>
        <?php $n++;}unset($n); ?>
